...
interface OutboundMethodModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function OutboundMethodModal({
  isOpen,
  onClose,
}: OutboundMethodModalProps) {
  const router = useRouter();

  const handleScanOutbound = () => {
    onClose();
    router.push("/outbound/scan");
  };
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <h3 className="text-lg font-semibold text-primary">스캔 출고</h3>
            </div>
            <svg
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <h3 className="text-lg font-semibold text-primary">직접 출고</h3>
            </div>
            <svg
              className="w-5 h-5 text-accent-soft group-hover:text-primary transition-colors"
              fill="none"
