...
export default function EditSupplierPage() {
  ...
  const loadSupplier = async () => {
    try {
      ...
      setError('공급업체 정보를 불러오는데 실패했습니다.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await suppliersAPI.update(supplierId, formData);
      ...
      setError('공급업체 수정에 실패했습니다. 다시 시도해주세요.');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (!supplier) {
    return (
      <div className="p-4">
        <div className="text-center py-8">
          <p className="text-red-600">공급업체를 찾을 수 없습니다.</p>
          <Link
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">공급업체 수정</h1>
      </div>
