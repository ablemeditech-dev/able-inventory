...
export default function EditClientPage() {
  ...
  const loadClientData = async () => {
    try {
      ...
      console.error('거래처 데이터 로딩 실패:', err);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await clientsAPI.update(clientId, formData);
      
      if (error) {
        throw error;
      }
      
      console.log('거래처 수정 성공:', data);
      ...
      setError('거래처 수정에 실패했습니다. 다시 시도해주세요.');
    } finally {
      setLoading(false);
    }
  };
